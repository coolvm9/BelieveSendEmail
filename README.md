...
```bash
BelieveSendEmail$ aws s3 mb s3://BUCKET_NAME
```

To prepare the application for deployment, use the `sam package` command.

```bash
BelieveSendEmail$ sam package \
...
```bash
BelieveSendEmail$ sam deploy \
    --template-file packaged.yaml \
    --stack-name BelieveSendEmail \
...
```bash
BelieveSendEmail$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
BelieveSendEmail$ sam logs -n HelloWorldFunction --stack-name BelieveSendEmail --tail
```
...
```bash
BelieveSendEmail$ cd HelloWorldFunction
...
```bash
BelieveSendEmail$ aws cloudformation delete-stack --stack-name BelieveSendEmail
BelieveSendEmail$ aws s3 rb s3://BUCKET_NAME
```
